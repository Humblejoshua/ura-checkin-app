...
// Config tab:
//   A1: Date        B1: (date)
//   A2: Session     B2: ROOM 1
//   A3: Session 1   B3: Session 1
//   A4: Session 2   B4: Session 2
//   A5: Session 3   B5: Session 3
...
//   F: Date | G: Room | H: Session1 | I: Session1 Time In | J: Session2
//   K: Session2 Time In | L: Session3 | M: Session3 Time In | N: Breakfast
...
function sheetFieldName(field) {
  var map = {
    'session1': 'Session1',
    'session1timein': 'Session1 Time In',
    'session2': 'Session2',
    'session2timein': 'Session2 Time In',
    'session3': 'Session3',
    'session3timein': 'Session3 Time In',
    'room': 'Room',
    'breakfast': 'Breakfast',
    'lunch': 'Lunch'
  };
...
function getConfig() {
  ...
  return {
    ...
    session1: String(sheet.getRange('B3').getValue() || 'Session 1'),
    session2: String(sheet.getRange('B4').getValue() || 'Session 2'),
    session3: String(sheet.getRange('B5').getValue() || 'Session 3'),
...
function getStats() {
  ...
  for (var i = 1; i < info.data.length; i++) {
    ...
    stats.total++;
    if (info.data[i][7]) stats.session1++;    // H: Session1
    if (info.data[i][9]) stats.session2++;    // J: Session2
    if (info.data[i][11]) stats.session3++;   // L: Session3
...
function initSheet() {
  ...
  learners.clear();
  var headers = [['S/N', 'Index Number', 'Candidate Name', 'Staff ID', 'Group', 'Date', 'Room', 'Session1', 'Session1 Time In', 'Session2', 'Session2 Time In', 'Session3', 'Session3 Time In', 'Breakfast', 'Lunch', 'Device', 'Timestamp']];
  ...
  expAtt.getRange('F3').setFormula('=IF(Config!B3="","Session 1",Config!B3)');
  expAtt.getRange('H3').setFormula('=IF(Config!B4="","Session 2",Config!B4)');
  expAtt.getRange('J3').setFormula('=IF(Config!B5="","Session 3",Config!B5)');
  for (var r = 0; r < 600; r++) {
    var row = r + 4;
    ...
    expAtt.getRange(row, 9).setValue('=IF(Learners!K' + (r + 2) + '="","",Learners!K' + (r + 2) + ')');
    expAtt.getRange(row, 10).setValue('=IF(Learners!L' + (r + 2) + '="","",Learners!L' + (r + 2) + ')');
    expAtt.getRange(row, 11).setValue('=IF(Learners!M' + (r + 2) + '="","",Learners!M' + (r + 2) + ')');
  }
...
function onOpen() {
  SpreadsheetApp.getUi()
    ...
    .addSeparator()
    .addItem('Setup QR Email Tab', 'setupQREmailTab')
    .addItem('Send QR Emails', 'sendQREmails')
    .addToUi();
}
...
function updateExportHeadersUI() {
  ...
  var s1 = String(config.getRange('B3').getValue() || 'Session 1');
  var s2 = String(config.getRange('B4').getValue() || 'Session 2');
  var s3 = String(config.getRange('B5').getValue() || 'Session 3');

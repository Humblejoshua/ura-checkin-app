...
function getStats(params) {
  ...
  for (var i = 1; i < data.length; i++) {
    if (data[i][6]) stats.timeIn++;     // G: Time In
    if (data[i][7]) stats.module1++;    // H: Module1
    if (data[i][8]) stats.module2++;    // I: Module2
    if (data[i][9]) stats.module3++;    // J: Module3
    if (data[i][10]) stats.breakfast++; // K: Breakfast
    if (data[i][11]) stats.lunch++;     // L: Lunch
  }
...
function saveRecord(params) {
  ...
  for (var i = 1; i < data.length; i++) {
    if (String(data[i][staffIdCol]).trim() === staffId) {
      // If date matches, found exact row
      if (recordDate && String(data[i][dateCol]).trim() === recordDate) {
        targetRow = i + 1;
        break;
      }
      // If no date specified, use first match (backward compat)
      if (!recordDate && targetRow === -1) {
        targetRow = i + 1;
      }
    }
  }

  // If no row found for this Staff ID + Date, create one
  if (targetRow === -1) {
    // Find any existing row for this Staff ID to copy learner info
    ...
    newRow[headers.indexOf('S/N')] = learnerInfo[headers.indexOf('S/N')];
    newRow[headers.indexOf('Index Number')] = learnerInfo[headers.indexOf('Index Number')];
    newRow[headers.indexOf('Candidate Name')] = learnerInfo[headers.indexOf('Candidate Name')];
    newRow[staffIdCol] = staffId;
    newRow[headers.indexOf('Group')] = learnerInfo[headers.indexOf('Group')];
    if (dateCol >= 0 && recordDate) newRow[dateCol] = recordDate;
    newRow[colIdx] = value;
    newRow[headers.indexOf('Device')] = device;
    newRow[headers.indexOf('Timestamp')] = new Date();
    sheet.appendRow(newRow);
    targetRow = sheet.getLastRow();

    var name = String(learnerInfo[headers.indexOf('Candidate Name')] || staffId);
  ...
  // Row found — check if already set
...
function addLearner(params) {
  ...
  for (var i = 1; i < data.length; i++) {
    if (String(data[i][staffIdCol]).trim() === staffId) {
      if (recordDate && dateCol >= 0 && String(data[i][dateCol]).trim() === recordDate) {
  ...
  if (dateCol >= 0 && recordDate) row[dateCol] = recordDate;
...
function initSheet() {
  ...
  // Dynamic module name headers from Config
  ...
  for (var r = 0; r < 600; r++) {
    ...
    expAtt.getRange(row, 5).setValue('=IF(Learners!G' + (r + 2) + '="","",Learners!G' + (r + 2) + ')');  // Time In
    expAtt.getRange(row, 6).setValue('=IF(Learners!H' + (r + 2) + '="","",Learners!H' + (r + 2) + ')');  // Module1
    expAtt.getRange(row, 7).setValue('=IF(Learners!I' + (r + 2) + '="","",Learners!I' + (r + 2) + ')');  // Module2
    expAtt.getRange(row, 8).setValue('=IF(Learners!J' + (r + 2) + '="","",Learners!J' + (r + 2) + ')');  // Module3
  }
  ...
  for (var r2 = 0; r2 < 600; r2++) {
    ...
    expMeals.getRange(row2, 4).setValue('=IF(Learners!K' + (r2 + 2) + '="","",Learners!K' + (r2 + 2) + ')');  // Breakfast
    expMeals.getRange(row2, 5).setValue('=IF(Learners!L' + (r2 + 2) + '="","",Learners!L' + (r2 + 2) + ')');  // Lunch
  }
...
function onOpen() {
  SpreadsheetApp.getUi()
    ...
    .item('Add Date Column (migrate)', 'migrateAddDateColumn')
...
// ── UI: Migrate existing Learners tab to add Date column ──

function migrateAddDateColumn() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var learners = ss.getSheetByName('Learners');
  if (!learners) { SpreadsheetApp.getUi().alert('Learners tab not found'); return; }
  var headers = learners.getRange(1, 1, 1, learners.getLastColumn()).getValues()[0];
  if (headers.indexOf('Date') >= 0) { SpreadsheetApp.getUi().alert('Date column already exists'); return; }
  learners.insertColumnAfter(5); // Insert after Group (col E)
  learners.getRange(1, 6).setValue('Date').setFontWeight('bold').setBackground('#0265B1').setFontColor('white');
  SpreadsheetApp.getUi().alert('Date column added at column F. Existing data shifted right.');
}

// ── UI: Update Export Attendance headers with module names from Config ──

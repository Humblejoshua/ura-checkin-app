...
//   F: Date | G: Module1 | H: Module1 Time In | I: Module2 | J: Module2 Time In
//   K: Module3 | L: Module3 Time In | M: Breakfast | N: Lunch | O: Device | P: Timestamp
//
...
function sheetFieldName(field) {
  var map = {
    'module1': 'Module1',
    'module1timein': 'Module1 Time In',
    'module2': 'Module2',
    'module2timein': 'Module2 Time In',
    'module3': 'Module3',
    'module3timein': 'Module3 Time In',
    'breakfast': 'Breakfast',
    'lunch': 'Lunch'
  };
...
function updateConfig(params) {
  ...
  if (expAtt) {
    if (params.module1) expAtt.getRange('E3').setValue(params.module1);
    if (params.module2) expAtt.getRange('G3').setValue(params.module2);
    if (params.module3) expAtt.getRange('I3').setValue(params.module3);
  }
...
function getStats() {
  var sheet = getLearnersSheet_();
  if (!sheet) return { total: 0, module1: 0, module2: 0, module3: 0, breakfast: 0, lunch: 0 };
  ...
  var info = getHeaders_(sheet);
  if (info.data.length < 2) return { total: 0, module1: 0, module2: 0, module3: 0, breakfast: 0, lunch: 0 };

  var dateCol = info.headers.indexOf('Date');
  var stats = { total: 0, module1: 0, module2: 0, module3: 0, breakfast: 0, lunch: 0 };
  ...
  for (var i = 1; i < info.data.length; i++) {
    ...
    stats.total++;
    if (info.data[i][6]) stats.module1++;    // G: Module1
    if (info.data[i][8]) stats.module2++;    // I: Module2
    if (info.data[i][10]) stats.module3++;   // K: Module3
    if (info.data[i][12]) stats.breakfast++; // M: Breakfast
    if (info.data[i][13]) stats.lunch++;     // N: Lunch
  }
...
function initSheet() {
  ...
  // Learners tab — 16 columns
  var learners = ss.getSheetByName('Learners');
  if (!learners) {
    learners = ss.insertSheet('Learners');
  }
  learners.clear();
  var headers = [['S/N', 'Index Number', 'Candidate Name', 'Staff ID', 'Group', 'Date', 'Module1', 'Module1 Time In', 'Module2', 'Module2 Time In', 'Module3', 'Module3 Time In', 'Breakfast', 'Lunch', 'Device', 'Timestamp']];
  learners.getRange(1, 1, 1, 16).setValues(headers);
  learners.getRange(1, 1, 1, 16).setFontWeight('bold').setBackground('#0265B1').setFontColor('white');
  ...
  // Export Attendance tab — 10 columns (Module + Time In per module)
  ...
  expAtt.getRange('A1:J1').merge();
  expAtt.getRange('A2').setValue('=IF(Config!B1="","","Date: "&TEXT(Config!B1,"dddd, d mmmm yyyy"))');
  expAtt.getRange('A2:J2').merge();
  var attExportHeaders = [['S/N', 'Index Number', 'Candidate Name', 'Staff ID', '', 'Time In', '', 'Time In', '', 'Time In']];
  expAtt.getRange(3, 1, 1, 10).setValues(attExportHeaders);
  expAtt.getRange(3, 1, 1, 10).setFontWeight('bold').setBackground('#0265B1').setFontColor('white');
  expAtt.getRange('E3').setFormula('=IF(Config!B3="","Module 1",Config!B3)');
  expAtt.getRange('G3').setFormula('=IF(Config!B4="","Module 2",Config!B4)');
  expAtt.getRange('I3').setFormula('=IF(Config!B5="","Module 3",Config!B5)');
  for (var r = 0; r < 600; r++) {
    var row = r + 4;
  ...
  for (var r2 = 0; r2 < 600; r2++) {
    ...
    expMeals.getRange(row2, 4).setValue('=IF(Learners!M' + (r2 + 2) + '="","",Learners!M' + (r2 + 2) + ')');
    expMeals.getRange(row2, 5).setValue('=IF(Learners!N' + (r2 + 2) + '="","",Learners!N' + (r2 + 2) + ')');
  }
...
function exportAttendanceCSV() {
  ...
  var csv = 'S/N,Index Number,Candidate Name,Staff ID,Date,' + m1 + ',' + m1 + ' Time In,' + m2 + ',' + m2 + ' Time In,' + m3 + ',' + m3 + ' Time In\n';
  for (var i = 1; i < data.length; i++) {
    if (!data[i][0]) continue;
    if (!isWithinDateRange(data[i][15], startDate, endDate)) continue;
    csv += data[i][0] + ',"' + data[i][1] + '","' + data[i][2] + '","' + data[i][3] + '","' + (data[i][5] || '') + '","' + (data[i][6] || '') + '","' + (data[i][7] || '') + '","' + (data[i][8] || '') + '","' + (data[i][9] || '') + '","' + (data[i][10] || '') + '","' + (data[i][11] || '') + '"\n';
  }
...
function exportMealsCSV() {
  ...
  for (var i = 1; i < data.length; i++) {
    if (!data[i][0]) continue;
    if (!isWithinDateRange(data[i][15], startDate, endDate)) continue;
    csv += data[i][0] + ',"' + data[i][2] + '","' + data[i][3] + '","' + (data[i][5] || '') + '","' + (data[i][12] || '') + '","' + (data[i][13] || '') + '"\n';
  }
...
function updateExportHeadersUI() {
  ...
  expAtt.getRange('E3').setValue(m1);
  expAtt.getRange('G3').setValue(m2);
  expAtt.getRange('I3').setValue(m3);

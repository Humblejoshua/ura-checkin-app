...
//
// HOW IT WORKS:
// - Rows are created ON-DEMAND when a learner is scanned
// - No pre-seeded blank rows — if a learner doesn't show up,
//   no row exists for them that day
// - Each row = one learner + one date (Staff ID + Date = unique key)
// - The phone app sends learner info (name, sn, group) with every
//   save, so the API can create new rows with full details
//
// HOW TO SET UP:
...
//   A1: Date        B1: (date)
//   A2: Room        B2: ROOM 1
//   A3: Morning Session     B3: Tax Law
//   A4: Mid-morning Session B4: Ethics
//   A5: Afternoon Session   B5: Computer Skills
//   A6: Supervisor  B6: (name)
//   A7: Approved    B7: FALSE
//   A8: Approved By B8: (name)
//   A9: Export Start Date  B9: (date)
//   A10: Export End Date   B10: (date)
//
// Learners tab headers (Row 1) — FINAL LAYOUT:
//   A: S/N | B: Index Number | C: Candidate Name | D: Staff ID | E: Group
//   F: Date | G: Room | H: Morning | I: Morning Time In | J: Mid-morning
//   K: Mid-morning Time In | L: Afternoon | M: Afternoon Time In | N: Breakfast
//   O: Lunch | P: Device | Q: Timestamp
//
//   Only learners who were scanned appear — one row per learner per day.
//   Staff ID + Date together form the unique row identifier.
//
...
// Normalize any value (Date object or string) to "yyyy-MM-dd" for consistent comparison
function normalizeDate_(val) {
  if (!val) return '';
  if (val instanceof Date) {
    try { return Utilities.formatDate(val, Session.getScriptTimeZone(), 'yyyy-MM-dd'); } catch(e) { return ''; }
  }
  ...
  var d = new Date(s);
  if (!isNaN(d.getTime())) {
    try { return Utilities.formatDate(d, Session.getScriptTimeZone(), 'yyyy-MM-dd'); } catch(e) { return s; }
  }
  return s;
}
...
function findRowByStaffIdAndDate_(sheet, staffId, recordDate, dateCol, staffIdCol) {
  ...
  var data = sheet.getDataRange().getValues();
  var normalizedTarget = normalizeDate_(recordDate);
  for (var i = 1; i < data.length; i++) {
    if (String(data[i][staffIdCol]).trim() === staffId) {
      if (dateCol >= 0 && normalizedTarget) {
        if (normalizeDate_(data[i][dateCol]) === normalizedTarget) {
...
function reloadColIndexes_(sheet, field) {
  ...
  return {
    ...
    roomCol: info.headers.indexOf('Room'),
...
function sheetFieldName(field) {
  var map = {
    'session1': 'Morning',
    'session1timein': 'Morning Time In',
    'session2': 'Mid-morning',
    'session2timein': 'Mid-morning Time In',
    'session3': 'Afternoon',
    'session3timein': 'Afternoon Time In',
    'room': 'Room',
    'breakfast': 'Breakfast',
    'lunch': 'Lunch'
  };
...
function getLearners(params) {
  ...
  var dateHeaderIdx = headers.indexOf('Date');
  for (var i = 1; i < info.data.length; i++) {
    ...
    for (var j = 0; j < headers.length; j++) {
      ...
      if (j === dateHeaderIdx) val = normalizeDate_(val);
      else val = String(val);
      obj[headers[j]] = val;
    }
    learners.push(obj);
...
function getConfig() {
  ...
  return {
    ...
    session1: String(sheet.getRange('B3').getValue() || 'Morning'),
    session2: String(sheet.getRange('B4').getValue() || 'Mid-morning'),
    session3: String(sheet.getRange('B5').getValue() || 'Afternoon'),
...
// ── POST: Update session names in Config ──
...
function updateConfig(params) {
  ...
  if (params.session1) sheet.getRange('B3').setValue(params.session1);
  if (params.session2) sheet.getRange('B4').setValue(params.session2);
  if (params.session3) sheet.getRange('B5').setValue(params.session3);
  ...
  // Update Export Attendance tab headers with session names
  var expAtt = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Export Attendance');
  if (expAtt) {
    if (params.session1) expAtt.getRange('F3').setValue(params.session1);
    if (params.session2) expAtt.getRange('H3').setValue(params.session2);
    if (params.session3) expAtt.getRange('J3').setValue(params.session3);
  }
...
function getStats() {
  var sheet = getLearnersSheet_();
  if (!sheet) return { total: 0, session1: 0, session2: 0, session3: 0, breakfast: 0, lunch: 0 };
  ...
  var info = getHeaders_(sheet);
  if (info.data.length < 2) return { total: 0, session1: 0, session2: 0, session3: 0, breakfast: 0, lunch: 0 };

  var dateCol = info.headers.indexOf('Date');
  var stats = { total: 0, session1: 0, session2: 0, session3: 0, breakfast: 0, lunch: 0 };

  var normalizedTarget = normalizeDate_(targetDate);
  for (var i = 1; i < info.data.length; i++) {
    if (dateCol >= 0 && normalizedTarget) {
      if (normalizeDate_(info.data[i][dateCol]) !== normalizedTarget) continue;
    }
    stats.total++;
    if (info.data[i][7]) stats.session1++;    // H: Morning
    if (info.data[i][9]) stats.session2++;    // J: Mid-morning
    if (info.data[i][11]) stats.session3++;   // L: Afternoon
    if (info.data[i][13]) stats.breakfast++;  // N: Breakfast
    if (info.data[i][14]) stats.lunch++;      // O: Lunch
  }
...
function saveRecord(params) {
  ...
  // Optional learner info (from phone's SEED_LEARNERS) for creating new rows
  var providedName = String(params.name || '').trim();
  var providedSn = String(params.sn || '').trim();
  var providedIdx = String(params.indexNumber || '').trim();
  var providedGroup = String(params.group || '').trim();
  var providedRoom = String(params.room || '').trim();
  ...
  if (cols.colIdx === -1) return { error: 'Field not found: ' + sheetFieldName(field) };
  if (cols.dateCol === -1) return { error: 'Date column not found' };

  // Find existing row for this Staff ID + Date
  var targetRow = findRowByStaffIdAndDate_(sheet, staffId, recordDate, cols.dateCol, cols.staffIdCol);

  // If no row found, create one with learner info from params or old row
  if (targetRow === -1) {
    ...
    // Prefer old row data, fall back to provided params
    if (cols.snCol >= 0) newRow[cols.snCol] = oldRow ? oldRow[cols.snCol] : providedSn;
    if (cols.idxCol >= 0) newRow[cols.idxCol] = oldRow ? oldRow[cols.idxCol] : providedIdx;
    if (cols.nameCol >= 0) newRow[cols.nameCol] = oldRow ? oldRow[cols.nameCol] : providedName;
    if (cols.staffIdCol >= 0) newRow[cols.staffIdCol] = staffId;
    if (cols.groupCol >= 0) newRow[cols.groupCol] = oldRow ? oldRow[cols.groupCol] : providedGroup;
    if (cols.roomCol >= 0) newRow[cols.roomCol] = providedRoom;
    newRow[cols.dateCol] = recordDate;
    ...
    var displayName = providedName || (oldRow ? oldRow[cols.nameCol] : staffId);
    logToAudit(field.toUpperCase(), device, displayName, field + ': ' + value + ' (new row)');
    return { success: true, staffId: staffId, name: displayName, field: field, value: value, newRow: true };
  }
  ...
  // Update room, device and timestamp
  if (cols.roomCol >= 0 && providedRoom) sheet.getRange(targetRow, cols.roomCol + 1).setValue(providedRoom);
...
function addLearner(params) {
  ...
  // Check duplicate by Staff ID + Date (allow same learner on different days)
  var existingRow = findRowByStaffIdAndDate_(sheet, staffId, recordDate, cols.dateCol, cols.staffIdCol);
  if (existingRow > 0) {
  ...
  if (cols.roomCol >= 0) row[cols.roomCol] = room;
  if (cols.dateCol >= 0) row[cols.dateCol] = recordDate;
...
function checkDuplicate(params) {
  ...
  for (var i = 1; i < data.length; i++) {
    if (String(data[i][cols.staffIdCol]).trim() === staffId) {
      if (cols.dateCol >= 0 && normalizedTarget) {
        if (normalizeDate_(data[i][cols.dateCol]) !== normalizedTarget) continue;
      }
...
function initSheet() {
  ...
  config.getRange('A1:B10').setValues([
    ['Date', ''],
    ['Room', 'ROOM 1'],
    ['Morning Session', 'Tax Law'],
    ['Mid-morning Session', 'Ethics'],
    ['Afternoon Session', 'Computer Skills'],
    ['Supervisor', ''],
    ['Approved', false],
    ['Approved By', ''],
    ['Export Start Date', ''],
  ...
  // Learners tab — 17 columns
  var learners = ss.getSheetByName('Learners');
  if (!learners) {
    learners = ss.insertSheet('Learners');
  }
  learners.clear();
  var headers = [['S/N', 'Index Number', 'Candidate Name', 'Staff ID', 'Group', 'Date', 'Room', 'Morning', 'Morning Time In', 'Mid-morning', 'Mid-morning Time In', 'Afternoon', 'Afternoon Time In', 'Breakfast', 'Lunch', 'Device', 'Timestamp']];
  learners.getRange(1, 1, 1, 17).setValues(headers);
  learners.getRange(1, 1, 1, 17).setFontWeight('bold').setBackground('#0265B1').setFontColor('white');
  ...
  // Export Attendance tab — 11 columns (Room + Session + Time In per session)
  var expAtt = ss.getSheetByName('Export Attendance');
  if (!expAtt) {
    expAtt = ss.insertSheet('Export Attendance');
  }
  expAtt.clear();
  expAtt.getRange('A1').setValue('Tax Academy On-Boarding 2026');
  expAtt.getRange('A1').setFontSize(14).setFontWeight('bold').setFontColor('#0265B1');
  expAtt.getRange('A1:K1').merge();
  expAtt.getRange('A2').setValue('=IF(Config!B1="","","Date: "&TEXT(Config!B1,"dddd, d mmmm yyyy"))');
  expAtt.getRange('A2:K2').merge();
  var attExportHeaders = [['S/N', 'Index Number', 'Candidate Name', 'Staff ID', 'Room', '', 'Time In', '', 'Time In', '', 'Time In']];
  expAtt.getRange(3, 1, 1, 11).setValues(attExportHeaders);
  expAtt.getRange(3, 1, 1, 11).setFontWeight('bold').setBackground('#0265B1').setFontColor('white');
  expAtt.getRange('F3').setFormula('=IF(Config!B3="","Morning",Config!B3)');
  expAtt.getRange('H3').setFormula('=IF(Config!B4="","Mid-morning",Config!B4)');
  expAtt.getRange('J3').setFormula('=IF(Config!B5="","Afternoon",Config!B5)');
  var attColLetters = ['A', 'B', 'C', 'D', 'G', 'H', 'I', 'J', 'K', 'L', 'M'];
  for (var r = 0; r < 600; r++) {
    var row = r + 4;
    var ln = r + 2;
    for (var c = 0; c < attColLetters.length; c++) {
      expAtt.getRange(row, c + 1).setValue(
        '=IF(AND(Learners!A' + ln + '<>"",Learners!F' + ln + '<>"",Config!$B$1<>""),IF(TEXT(Learners!F' + ln + ',"yyyy-mm-dd")=TEXT(Config!$B$1,"yyyy-mm-dd"),Learners!' + attColLetters[c] + ln + ',""),"")'
      );
    }
  }
  ...
  expMeals.clear();
  expMeals.getRange('A1').setValue('Tax Academy On-Boarding 2026');
  ...
  for (var r2 = 0; r2 < 600; r2++) {
    ...
    expMeals.getRange(row2, 4).setValue('=IF(Learners!N' + (r2 + 2) + '="","",Learners!N' + (r2 + 2) + ')');
    expMeals.getRange(row2, 5).setValue('=IF(Learners!O' + (r2 + 2) + '="","",Learners!O' + (r2 + 2) + ')');
  }
...
function exportAttendanceCSV() {
  ...
  var s1 = config.getRange('B3').getValue();
  var s2 = config.getRange('B4').getValue();
  var s3 = config.getRange('B5').getValue();
  ...
  var csv = 'S/N,Index Number,Candidate Name,Staff ID,Date,Room,' + s1 + ',' + s1 + ' Time In,' + s2 + ',' + s2 + ' Time In,' + s3 + ',' + s3 + ' Time In\n';
  for (var i = 1; i < data.length; i++) {
    if (!data[i][0]) continue;
    if (!isWithinDateRange(data[i][16], startDate, endDate)) continue;
    csv += data[i][0] + ',"' + data[i][1] + '","' + data[i][2] + '","' + data[i][3] + '","' + (data[i][5] || '') + '","' + (data[i][6] || '') + '","' + (data[i][7] || '') + '","' + (data[i][8] || '') + '","' + (data[i][9] || '') + '","' + (data[i][10] || '') + '","' + (data[i][11] || '') + '","' + (data[i][12] || '') + '"\n';
  }
  ...
  var title = 'Tax Academy On-Boarding 2026';
...
function exportMealsCSV() {
  ...
  for (var i = 1; i < data.length; i++) {
    if (!data[i][0]) continue;
    if (!isWithinDateRange(data[i][16], startDate, endDate)) continue;
    csv += data[i][0] + ',"' + data[i][2] + '","' + data[i][3] + '","' + (data[i][5] || '') + '","' + (data[i][13] || '') + '","' + (data[i][14] || '') + '"\n';
  }
...
function onOpen() {
  SpreadsheetApp.getUi()
    ...
    .addItem('Update Export Headers', 'updateExportHeadersUI')
    .addItem('Export Attendance CSV', 'exportAttendanceCSV')
    .addItem('Export Meals CSV', 'exportMealsCSV')
    .addItem('Approve & Lock', 'approveAndLockUI')
    .addToUi();
}
...
function updateExportHeadersUI() {
  ...
  var s1 = String(config.getRange('B3').getValue() || 'Morning');
  var s2 = String(config.getRange('B4').getValue() || 'Mid-morning');
  var s3 = String(config.getRange('B5').getValue() || 'Afternoon');
  expAtt.getRange('F3').setValue(s1);
  expAtt.getRange('H3').setValue(s2);
  expAtt.getRange('J3').setValue(s3);
  SpreadsheetApp.getUi().alert('Headers updated to: ' + s1 + ' | ' + s2 + ' | ' + s3);
}
